...
const AddMealModal = ({ isOpen, onClose, onAddMeal }) => {
    ...
    return (
        <div className="fixed inset-0 bg-black z-50 transition-opacity animate-fade-in-scale">
            ...
            <div className="relative w-full h-full">
                ...
                <div className="absolute inset-0 flex flex-col justify-between">
                    ...
                    <div className="flex justify-between items-start p-4 bg-gradient-to-b from-black/60 to-transparent w-full">
                        <h2 className="text-2xl font-bold text-white pt-2">Scan a Meal</h2>
                        <button onClick={handleClose} className="p-3 text-white bg-black/50 hover:bg-black/70 rounded-full transition-colors z-20">
                            <XIcon />
                        </button>
                    </div>

                    {/* Conditionally rendered result panel at the bottom */}
                    {apiResult && !isLoading && (
                        <div className="p-6 bg-white dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 rounded-t-2xl text-center animate-fade-in-up space-y-4">
               ...
               {!apiResult && !isLoading && !capturedImage && (
                ...
                {isLoading && <div className="absolute inset-0 flex items-center justify-center bg-black/70"><Loader /></div>}
                {error && !isLoading && (
                    <div className="absolute inset-0 flex items-center justify-center bg-black/70 p-4">
                        <div className="text-center text-white bg-slate-800/80 p-6 rounded-lg">
                            <p className="font-semibold mb-4">{error}</p>
                            <button onClick={handleRetake} className="bg-white text-black font-bold py-2 px-4 rounded-lg hover:bg-gray-200 transition-colors flex items-center justify-center gap-2 mx-auto">
